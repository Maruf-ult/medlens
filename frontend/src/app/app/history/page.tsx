...
import type { AnalysisRecord, AnalysisDetailRecord } from "@/lib/api";
...
function DetailView({ detail, onClose }: { detail: AnalysisDetailRecord; onClose: () => void }) {
  ...
  const donutData = [
    { name: "Normal",    value: findings.filter((f: {severity: string}) => f.severity === "normal").length,   fill: SEVERITY_COLORS.normal   },
    { name: "Attention", value: findings.filter((f: {severity: string}) => f.severity === "warning").length,  fill: SEVERITY_COLORS.warning  },
    { name: "Critical",  value: findings.filter((f: {severity: string}) => f.severity === "critical").length, fill: SEVERITY_COLORS.critical },
  ].filter(d => d.value > 0);

  const barData = findings.map((f: {title: string; severity: "normal" | "warning" | "critical"}) => ({
  ...
  const radialData = [
    { name: "Normal",    value: Math.round((findings.filter((f: {severity: string}) => f.severity === "normal").length   / total) * 100), fill: SEVERITY_COLORS.normal   },
    { name: "Attention", value: Math.round((findings.filter((f: {severity: string}) => f.severity === "warning").length  / total) * 100), fill: SEVERITY_COLORS.warning  },
    { name: "Critical",  value: Math.round((findings.filter((f: {severity: string}) => f.severity === "critical").length / total) * 100), fill: SEVERITY_COLORS.critical },
  ];
      ...
      <div className="min-h-screen px-4 py-8">
        <div className="max-w-4xl mx-auto bg-white dark:bg-surface-900 rounded-2xl shadow-2xl">
          ...
          <div className="flex items-start justify-between p-6 border-b border-surface-200 dark:border-surface-700">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg flex items-center justify-center text-surface-400 hover:text-surface-700 hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors"
            >
          ...
          <div className="p-6">
            ...
            {activeTab === "summary" && (
              <div className="space-y-5">
                ...
                <div className="flex flex-wrap gap-4 pt-4 border-t border-surface-100 dark:border-surface-700">
                  {[
                    { label: "Normal",    count: findings.filter((f: {severity: string}) => f.severity === "normal").length,   color: "text-success-600",  bg: "bg-success-50 dark:bg-success-950"  },
                    { label: "Attention", count: findings.filter((f: {severity: string}) => f.severity === "warning").length,  color: "text-warning-600",  bg: "bg-warning-50 dark:bg-warning-950"  },
                    { label: "Critical",  count: findings.filter((f: {severity: string}) => f.severity === "critical").length, color: "text-danger-600",   bg: "bg-danger-50 dark:bg-danger-950"    },
                  ].map((s) => (
                ...
                <div className={cn("p-5 rounded-xl border", "bg-surface-50 dark:bg-surface-800", "border-surface-200 dark:border-surface-700")}>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={barData} barSize={24}>
                      ...
                      <YAxis domain={[0,3]} ticks={[1,2,3]} tickFormatter={(v) => v===1?"Normal":v===2?"Warning":"Critical"} tick={{ fontSize: 9, fill: "#94a3b8" }} axisLine={false} tickLine={false} width={55} />
                      <Bar dataKey="value" radius={[4,4,0,0]}>
                        {barData.map((entry: {fill: string}, i: number) => <Cell key={i} fill={entry.fill} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                </div>
            ...
            {activeTab === "findings" && (
              <div className="space-y-3">
                {findings.map((finding: {severity: "normal"|"warning"|"critical"; title: string; value?: string; detail: string; reference_range?: string}, index: number) => (
            ...
            {activeTab === "questions" && (
              <div className="space-y-3">
                {(Array.isArray(detail.doctor_questions) ? detail.doctor_questions : []).map((q: string, i: number) => (
...
export default function HistoryPage() {
  ...
  const getUrgencyColor = (score: number) => score <= 3 ? "text-success-600 dark:text-success-400" : score <= 6 ? "text-warning-600 dark:text-warning-400" : "text-danger-600 dark:text-danger-400";
    ...
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-surface-500 dark:text-surface-400">{history.length} total analyses saved</p>
        </div>
      ...
      {error && (
        <div className="p-4 rounded-xl bg-danger-50 dark:bg-danger-950 border border-danger-200 text-danger-700 text-sm">{error}</div>
      )}

      {!isLoading && !error && history.length === 0 && (
        <div className={cn("flex flex-col items-center justify-center py-20", "bg-white dark:bg-surface-800", "rounded-2xl border border-surface-200 dark:border-surface-700")}>
          <div className="text-5xl mb-4">📋</div>
          <h3 className="text-lg font-semibold text-surface-900 dark:text-surface-100 mb-2">No analyses yet</h3>
          <p className="text-surface-400 text-sm mb-6 text-center max-w-sm">Your analyzed reports will appear here automatically.</p>
          <Link href="/app/analyze">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      ...
                      <span className="text-xs text-surface-400">{item.overall_status}</span>
                      {item.phi_detected && <span className="text-xs text-success-600 dark:text-success-400">🔒 PHI removed</span>}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 w-8 h-8 rounded-lg flex items-center justify-center text-surface-400 hover:text-danger-500 hover:bg-danger-50 dark:hover:bg-danger-950 transition-all"
                  >
